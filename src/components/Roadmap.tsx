...
const Roadmap = () => {
  ...
  return <section id="roadmap" className="bg-white py-10">
        ...
        <div className="max-w-4xl mx-auto" ref={ref as React.RefObject<HTMLDivElement>}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
            ...
            <div className="space-y-6 relative" ref={lineContainerRef}>
              {/* Connecting line is now behind the content with z-index: -1 */}
              <div className="connecting-line"></div>
              <div 
                className="connecting-line-filled" 
                style={{
                  height: `${lineProgress}%`
                }}
              ></div>
              ...
              {storySteps.map((step, index) => <div key={index} className={`relative pl-12 py-4 transition-all duration-500 cursor-pointer rounded-lg ${activeStoryIndex === index ? 'scale-105 bg-gray-50 shadow-sm border border-gray-100' : 'opacity-70 hover:opacity-90'}`} onClick={() => setActiveStoryIndex(index)}>
                  <div className={`absolute left-0 p-2 rounded-full z-10 ${activeStoryIndex === index ? 'animate-pulse-subtle' : ''}`} style={{
                backgroundColor: step.color + '20'
              }}>
                    {step.icon}
                  </div>
                  <h4 className="text-xl font-bold mb-2 relative z-10">{step.title}</h4>
                  <p className="text-gray-700 relative z-10">{step.description}</p>
                  
                  {activeStoryIndex === index && <div className={`mt-3 animate-fade-in flex gap-2 flex-wrap relative z-10`}>
                      {index === 0 && <>
