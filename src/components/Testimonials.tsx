  ...
  return <section className="py-24 bg-gray-50">
      <div className="section-container">
        ...
        <div className="relative max-w-4xl mx-auto" onMouseEnter={() => setIsPaused(true)} onMouseLeave={() => setIsPaused(false)}>
          ...
          <button onClick={handlePrev} className="absolute top-1/2 -translate-y-1/2 -left-4 md:-left-6 w-12 h-12 rounded-full bg-flowai-black text-flowai-white flex items-center justify-center shadow-lg transform transition-transform hover:scale-105 focus:outline-none">
            <ChevronLeft size={24} />
          </button>
          
          <button onClick={handleNext} className="absolute top-1/2 -translate-y-1/2 -right-4 md:-right-6 w-12 h-12 rounded-full bg-flowai-black text-flowai-white flex items-center justify-center shadow-lg transform transition-transform hover:scale-105 focus:outline-none">
          ...
          <div className="flex justify-center mt-8 gap-2">
            ...
            resetInterval();
          }} className={`w-2.5 h-2.5 rounded-full transition-all duration-300 ${activeIndex === index ? 'bg-flowai-black w-6' : 'bg-flowai-black/30 hover:bg-flowai-black/50'}`} aria-label={`Go to testimonial ${index + 1}`} />)}
          </div>
        </div>
      </div>
    </section>;
